...
# Порядок альтернатив — від довшої до коротшої, і без \b у кінці правила 1.
# Витяг тексту з RTF подекуди склеює слова ("Васильовичазалишити"), тому межі
# слова після по-батькові може не бути; а перша ж коротка альтернатива без
# такого впорядкування з'їдала б лише "ович" і лишала прізвище в тексті.
_PATRONYMIC = r"(?:овичем|овичу|овича|ович|йовичем|йовичу|йовича|йович|" \
              r"івною|івну|івні|івни|івна|ївною|ївну|ївні|ївни|ївна)"

RULES: list[tuple[str, re.Pattern[str]]] = [
    # Прізвище Ім'я По-батькові (у будь-якому відмінку)
    ("ПІБ", re.compile(rf"\b[{U}][{L}]+\s+[{U}][{L}]+\s+[{U}][{L}]+{_PATRONYMIC}")),
    # Ім'я По-батькові Прізвище
    ("ПІБ", re.compile(rf"\b[{U}][{L}]+\s+[{U}][{L}]+{_PATRONYMIC}\s+[{U}][{L}]+\b")),
    # По-батькові без прізвища поруч, але з іменем: "Віталія Васильовича"
    ("ПІБ", re.compile(rf"\b[{U}][{L}]+\s+[{U}][{L}]+{_PATRONYMIC}")),
    # В.М. Соколов / В. М. Соколов / С. Бакуліна / «О.В Білоус» — друга
    # ініціала подекуди набрана без крапки, і рядок проскакував перевірку.
